<script lang="ts">
  // ============================================================================
  // SERVER LOAD EXAMPLE PAGE
  // ============================================================================
  // 
  // DATA LANGSUNG DARI SERVER - Tidak perlu fetch()!
  // Data dari +page.server.ts load() function otomatis available di sini
  //
  // ============================================================================
  ...
  // Data otomatis dari server load function!
  export let data: PageData;
  
  // Reactive values dari data
  $: ({ users, emailUsers, totalUsers, loadedAt } = data);
  
  // Fungsi untuk refresh data (panggil load() lagi)
  function refreshData() {
    // Invalidate 'app:users' dependency, akan trigger load() ulang
  ...
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="text-center">
      ...
      <p class="text-gray-400">
        Data di-load DI SERVER, bukan dari browser!
      </p>
    ...
    <div class="flex justify-center">
      <button
        ...
        class="flex items-center gap-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition"
      >
        ...
        {#each users as user}
          <div class="px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              ...
              <!-- Delete Form dengan Form Action -->
              <form
                method="POST"
                action="?/deleteUser"
                use:enhance={() => {
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-red-900/20 border border-red-500/30 rounded-xl p-6">
        ...
        <pre class="text-sm text-gray-400 overflow-x-auto"><code>&lt;script&gt;
  // Page.svelte - Fetch dari browser
  let users = [];
    ...
    <div class="bg-blue-900/20 border border-blue-500/30 rounded-xl p-6">
      ...
      <ul class="space-y-2 text-gray-300">
        <li class="flex items-start gap-2">
          ...
          <span><strong>1 Request</strong> - Server render HTML lengkap dengan data</span>
        </li>
        <li class="flex items-start gap-2">
          <span class="text-green-400">✓</span>
          <span><strong>SEO Friendly</strong> - Google bisa index konten</span>
        </li>
        <li class="flex items-start gap-2">
          <span class="text-green-400">✓</span>
          <span><strong>No Loading State</strong> - User langsung lihat data</span>
        </li>
        <li class="flex items-start gap-2">
          <span class="text-green-400">✓</span>
          <span><strong>Type Safe</strong> - Data typed dari server</span>
        </li>
        <li class="flex items-start gap-2">
          <span class="text-green-400">✓</span>
          <span><strong>Secure</strong> - Bisa access secrets di server</span>
        </li>
      </ul>
    </div>
  </div>
